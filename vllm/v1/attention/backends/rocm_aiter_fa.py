...
class AiterFlashAttentionMetadataBuilder(
    ...
    def _maybe_allocate_shuffle_scale(self) -> None:
        """Lazily allocate the per-block scale buffer required by
        aiter.pa_fwd_asm under fp8 shuffle KV layout.

        pa_fwd_asm reads K_QScale / V_QScale as
        [num_blocks, num_kv_heads, block_size]; without this allocation it
        receives the scalar tensor([1.0]) from __init__ and SIGSEGVs on
        out-of-bounds pointer arithmetic. Allocated once per builder.

        The buffer is filled with ones because reshape_and_cache_shuffle_kernel
        currently hard-codes the quant scale to 1.0 on the write side
        (see the SHUFFLE branch ~line 145). When that kernel learns to track
        real per-block scales, this allocation must be replaced by a tensor
        that the write kernel and both the target's and drafter's read kernels
        share, otherwise reads will dequantize with stale ones.
        """
        if (
        ...
        if not attn_metadata.use_cascade:
            ...
            if num_decodes > 0:
                ...
                if decode_max_query_len > 1:
                    if rocm_aiter_ops.is_shuffle_kv_cache_enabled():
                        # MTP + shuffle KV: route multi-query decode straight
                        # to aiter.pa_fwd_asm. The asm kernel is the only
                        # reader that consumes shuffle layout natively;
                        # mha_v3 / unified_attention expect NHD storage and
                        # would produce garbage on shuffle-laid-out K/V.
                        # We bypass paged_attention_common to avoid its
                        # CU-count heuristic, which would silently route
                        # small batches to the HIP path (mtp=1 hardcoded).
                        # The asm path is causal-only.
                        ...
                        # query_start_loc[:num_decodes+1] is the cumulative
                        # Q-token count per decode sequence — exactly the
                        # qo_indptr layout pa_fwd_asm expects. Variable
                        # per-seq qlen is supported by the asm kernel.
